...
export function MintingMenuFullscreen({ isOpen, onClose }: MintingMenuFullscreenProps) {
  ...
  const [padImageData, setPadImageData] = useState<string | null>(null);

  // Capture pad content and screenshot when menu opens
  useEffect(() => {
    if (isOpen) {
      ...
      if (editorElement) {
        const content = (editorElement as HTMLTextAreaElement).textContent || (editorElement as HTMLTextAreaElement).value || '';
        setPadContent(content.slice(0, 100) + (content.length > 100 ? '...' : ''));
      }

      // Capture screenshot of the pad area
      const captureScreenshot = async () => {
        try {
          const padElement = document.querySelector('.lyrics-editor') || document.querySelector('main');
          if (padElement && (window as any).html2canvas) {
            const canvas = await (window as any).html2canvas(padElement, {
              width: 400,
              height: 400,
              scale: 0.5,
              backgroundColor: '#0B0F14'
            });
            setPadImageData(canvas.toDataURL());
          }
        } catch (error) {
      ...
      // Delay capture slightly to ensure menu is fully rendered
              ...
              <div className="flex-1 p-6 border-r" style={{ borderColor: 'var(--dropsource-border)' }}>
                ...
                <div className="mb-6">
                  <label 
                    ...
                    style={{ color: 'var(--dropsource-primary)', fontSize: 'var(--text-sm)' }}
                  >
                    Collectible Name *
                  </label>
                  <Input
                    ...
                    placeholder="Enter a name for your collectible"
                    className="dropsource-input"
                    maxLength={30}
                  />
                  <p style={{ color: 'var(--dropsource-tertiary)', fontSize: 'var(--text-xs)' }} className="mt-1">
                ...
                <div className="flex flex-col items-center justify-center h-2/3 space-y-6">
                  {/* Large Preview */}
                  <div 
                    className="w-64 h-64 rounded-2xl overflow-hidden transition-all duration-300 relative"
                    style={{
                  ...
                  >
                    {(window as any).stickerImageData ? (
                      <>
                        {/* Sticker image from Fabric.js canvas */}
                        <img 
                          src={(window as any).stickerImageData}
                          alt="Sticker Preview"
                        ...
                        <div className="relative z-10 p-6 h-full flex flex-col justify-end">
                          <div className="bg-black/50 backdrop-blur-sm rounded-lg p-4">
                            ...
                            >
                              {customName || 'Untitled Sticker'}
                            </h3>
                            <p 
                              className="text-white/90 text-xs leading-relaxed"
                              style={{ textShadow: '0 1px 2px rgba(0,0,0,0.8)' }}
                            >
                              {padContent || 'Your Original Creation'}
                            </p>
                          </div>
                        </div>
                      </>
                      ...
                      <div className="p-8 flex flex-col items-center justify-center h-full">
                        ...
                        <div className="text-center">
                          ...
                          >
                            {customName || 'Untitled Creation'}
                          </h3>
                          <p 
                            className="text-white/80 text-sm max-w-48 truncate"
                  ...
                  <div className="pt-6 space-y-4">
                    ...
                    <div className="flex gap-4">
                      ...
                      <Button
                        ...
                        disabled={!selectedRarity || isProcessing || !customName.trim()}
