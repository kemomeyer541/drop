...
import StreamBox from '../blocks/StreamBox';
import VideosBox from '../blocks/VideosBox';
import { 
...
function BlockFrame({ block, onDelete, onStyleChange, children }: BlockFrameProps) {
  ...
  return (
    ...
    >
      {/* Invisible drag handle for moving */}
      <div 
...
const ProfileCanvas = forwardRef<ProfileCanvasRef, ProfileCanvasProps>(({ 
  ...
  // Smart guides system
  const guides = useSmartGuides({ grid: 8, threshold: 10 });
  ...
  const updateBlock = (id: string, patch: Partial<Block>) => {
    console.log('updateBlock called:', { id, patch });
    setBlocks(prev => {
      const updated = prev.map(b => (b.id === id ? { ...b, ...patch } : b));
      console.log('Blocks updated:', updated.find(b => b.id === id));
  ...
  // Grid quantization is now handled by useSmartGuides
  ...
  return (
    <div className="w-full h-full relative">
      ...
      <div 
        ...
        style={{
          background: particleEffect && particleEffect !== 'none' ? 'transparent' : getCanvasBackground(canvasBackground),
      ...
      >
        <div id="profile-canvas-inner" ref={innerRef} className="profile-canvas-inner">
          ...
          {/* Particle Effects Layer - Above canvas sheet but below blocks */}
          ...
          {/* Guide lines would be rendered here if needed */}
          
          {blocks.map(block => (
            <Rnd
              key={`${block.id}-${block.w}-${block.h}-${block.x}-${block.y}`}
              bounds="parent"
              ...
              onDrag={(e, d) => {
                ...
                const moving = { id: block.id, x: d.x, y: d.y, w: block.w, h: block.h };
                const others = blocks.filter(b => b.id !== block.id).map(b => ({ id: b.id, x: b.x, y: b.y, w: b.w, h: b.h }));
                const { x, y } = guides.compute(moving, others, d.x, d.y);
                // Could show visual guides here if needed
              }}
              onResize={(e, dir, ref, delta, pos) => {
                console.log('Resize in progress:', { dir, width: ref.offsetWidth, height: ref.offsetHeight, pos });
                // Update the block state in real-time during resize
              ...
              onResizeStop={(e, dir, ref, delta, pos) => {
                ...
                console.log('Block updated with new dimensions:', { w: newWidth, h: newHeight });
                // Defer ensureFits to avoid render-time updates
                setTimeout(() => ensureFits(), 0);
              }}
              minWidth={360}
