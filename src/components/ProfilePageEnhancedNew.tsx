...
// Zero flair on Profile - commented out for complete cleanup
...
import AuroraOverlay from './profile/AuroraOverlay';
import ElementBlocklist from './profile/ElementBlocklist';
import DragBoard, { CardData } from './profile/DragBoard';
...
const AVAILABLE_BLOCKS: Array<{
  ...
  { type: 'video', icon: '📹', label: 'Videos', category: 'Media' },
  { type: 'stream', icon: '📡', label: 'Stream', category: 'Media' },
...
function ProfileCanvasInner({ onNavigate }: ProfilePageProps) {
  ...
  // Kill-switch: block mousemove listeners on Profile page
  React.useEffect(() => {
    const origAdd = window.addEventListener;
    const patched: typeof window.addEventListener = function (type: any, listener: any, options: any) {
      if (type === "mousemove") {
        // block hover-trail registrations on Profile
        return;
      }
      // @ts-ignore
  ...
  // New drag board state
  const [dragCards, setDragCards] = useState<CardData[]>([
    { id: "about", x: 40, y: 120, w: 360, h: 220, color: "#111827" },
    { id: "stats", x: 420, y: 120, w: 360, h: 220, color: "#0ea5e9" },
    { id: "media", x: 40, y: 360, w: 740, h: 420, color: "#111827" },
  ]);
  const [showDragBoard, setShowDragBoard] = useState(false);

  const updateCardColor = (id: string, hex: string) =>
    setDragCards(prev => prev.map(c => (c.id === id ? { ...c, color: hex } : c)));
  ...
  return (
    <div className="h-full overflow-hidden dropsource-bg dropsource-text-primary">
      {/* Element Blocklist - Alt+click to hide any element permanently */}
      <ElementBlocklist />
      {/* Aurora Effect - DISABLED to remove glow */}
      {/* <AuroraOverlay hue={205} opacity={0.32} /> */}
      {/* Header */}
      <header className="flex items-center justify-between p-4 border-b border-gray-700 bg-black/20" data-flair-ignore>
        ...
        <div className="flex items-center gap-2">
          <Button
            onClick={() => setShowDragBoard(!showDragBoard)}
            className="dropsource-btn-secondary"
          >
            <Grid3X3 className="w-4 h-4 mr-2" />
            {showDragBoard ? 'Hide Drag Board' : 'Show Drag Board'}
          </Button>
          <Button
      ...
      <div className="flex h-full">
        ...
        {!previewMode && (
          <div className="profile-sidebar border-r border-gray-700 dropsource-panel overflow-y-auto flex flex-col" data-flair-ignore>
            {/* Section Navigation */}
            <div className="p-4">
        ...
        >
          <div
            ...
            style={{ minHeight: 720 }}
            onPointerDown={(e) => {
              // Zero flair on Profile - no burst effects
          ...
          >
            ...
            <div id="profile-canvas-inner" className="relative" style={{ zIndex: 10 }}>
              ...
              {/* Drag Board - only shown when toggle is on */}
              {showDragBoard && (
                <div className="absolute inset-0 z-20">
                  <DragBoard
                    cards={dragCards}
                    onChange={setDragCards}
                    grid={8}
                    snapThreshold={10}
                  >
                    {({ card, style, bind }) => (
                      <ProfileCard
                        ...
                        onColor={(hex) => updateCardColor(card.id, hex)}
                        bind={bind}
                      >
                        {card.id === "media" ? (
                          <ProfileVideos />
                          ...
                          <div className="text-white/80">
                            ...
                            <p className="text-sm">This is a draggable card with enhanced snapping!</p>
                          </div>
                        ) : card.id === "stats" ? (
                          <div className="text-white/80">
                          ...
                          <div className="text-white/80">#{card.id}</div>
                        )}
                      </ProfileCard>
                    )}
                  </DragBoard>
