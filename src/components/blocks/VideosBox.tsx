import React, { useEffect, useRef, useState } from "react";

type VideoItem = { id: string; src: string; caption?: string };

const demo: VideoItem[] = [
  { id: "v1", src: "/videos/clip1.mp4", caption: "Studio rough cut" },
  { id: "v2", src: "/videos/clip2.mp4", caption: "Loop idea" },
  { id: "v3", src: "/videos/clip3.mp4", caption: "Lyric draft" },
];

const VideosBox: React.FC<{ items?: VideoItem[] }> = ({ items = demo }) => {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const vids = useRef<Map<string, HTMLVideoElement>>(new Map());
  const [active, setActive] = useState(items[0]?.id ?? null);

  useEffect(() => {
    if (!containerRef.current) return;
    const io = new IntersectionObserver(
      (entries) => {
        // choose the most visible item as active
        let best: { id: string; ratio: number } | null = null;
        for (const e of entries) {
          const id = (e.target as HTMLElement).dataset.id!;
          if (!best || e.intersectionRatio > best.ratio) best = { id, ratio: e.intersectionRatio };
        }
        if (best) setActive(best.id);
      },
      { root: containerRef.current, threshold: [0.25, 0.5, 0.75] }
    );

    const nodes = containerRef.current.querySelectorAll("[data-videocard='1']");
    nodes.forEach((n) => io.observe(n));
    return () => io.disconnect();
  }, [items]);

  useEffect(() => {
    vids.current.forEach((v, id) => {
      if (id === active) v.play().catch(() => {});
      else v.pause();
    });
  }, [active]);

  return (
    <div
      ref={containerRef}
      className="relative h-[calc(100vh-160px)] overflow-y-scroll snap-y snap-mandatory"
      style={{ scrollbarWidth: "none" }}
    >
      {items.map((it) => (
        <section
          key={it.id}
          data-id={it.id}
          data-videocard="1"
          className="h-[calc(100vh-160px)] snap-start relative flex items-center justify-center"
        >
          <video
            ref={(el) => { if (el) vids.current.set(it.id, el); }}
            src={it.src}
            className="h-full w-auto max-w-full rounded-2xl"
            muted
            playsInline
            loop
            preload="metadata"
            onClick={(e) => {
              const v = e.currentTarget;
              if (v.paused) v.play(); else v.pause();
            }}
          />
          {it.caption && (
            <div className="absolute bottom-6 left-6 right-6 text-white/95 text-sm bg-black/30 backdrop-blur-md px-3 py-2 rounded-xl">
              {it.caption}
            </div>
          )}
        </section>
      ))}
    </div>
  );
};

export default VideosBox;