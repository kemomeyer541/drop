...
export type Block = {
  id: string;
  type: 'header'|'chalkboard'|'new-drops'|'your-posts'|'links'|'collection'|'comments'|'friends'|'shop'|'donators'|'monthly-supporters'|'photos'|'video'|'stream'|'music-player'|'pinned-note'|'stats'|'events'|'poll'|'mini-games'|'custom';
  x: number; 
  y: number; 
  w: number; 
  h: number;
